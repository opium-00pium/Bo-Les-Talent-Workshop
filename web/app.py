        ...
        if request.method == 'GET':
            ...
            if bookid: 
                sql = """SELECT BookTitle,
                                ...
                                ImageM 
                           FROM Books 
                           WHERE BookID="{}" """.format(bookid)
                fetched_book_data = mysql.fetchone_db(sql) # 使用 fetchone_db 因为我们期望只有一条记录
                
                if fetched_book_data: 
                    book_details = fetched_book_data # 直接使用查询到的字典
                    if userid: 
                        update_recommend_book(userid, bookid)
                # else: book_details 会保持为 None，模板中需要处理这种情况

        if userid and bookid: 
            sql_rating = '''SELECT Rating FROM Bookrating 
                            where UserID="{}" and BookID="{}"'''.format(userid, bookid)
            score_data = mysql.fetchone_db(sql_rating)
            if score_data and score_data.get('Rating') is not None: 
                try:
                    score_value = int(score_data['Rating'])
                    score = math.ceil(score_value / 2)
                    if score > 10: score = 10
                except ValueError:
                    logger.warning(f"Could not convert rating '{score_data['Rating']}' to int for BookID {bookid}, UserID {userid}")
                    score = 0 
...
def order():
    ...
    try:
        sql = '''SELECT b.BookID,
                        b.BookTitle,
                        b.BookAuthor,
                        floor((b.PublicationYear-1000)/10) FROM (SELECT * from Cart ) a  
                        LEFT  JOIN  Books as b on a.BookID = b.BookID where a.UserID = "{}"
                        '''.format(userid)
...
def editinfo():
    ...
    try:
        if request.method == 'POST':
            location = request.form['location']  # <-- 修改：从表单获取 'location'
            age = request.form['age']
            try:
                # 修改SQL语句以更新Location字段
                sql = "UPDATE User SET Location='{}',Age= '{}' WHERE UserID='{}'".format(location, age, userid)
                mysql.exe(sql)
                # 修改日志记录中的变量名
                logger.info("UPDATE userinfo --> userid:{},location:{},age:{} ".format(userid, location, age))
            except Exception as e:
                mysql.rollback()
                # 修改日志记录中的变量名
                logger.exception("userid:{},location:{},age:{} UPDATE filed".format(userid, location, age))
            return redirect(url_for('user'))
    except Exception as e:
        logger.exception("edit user info error: {}".format(e)) # <-- 修改：更准确的日志信息
        return redirect(url_for('user'))
...
def addbook():
    ...
    try:
        if request.method == 'POST':
            ...
            sql = '''INSERT INTO Books (BookID,BookTitle,BookAuthor,PublicationYear,Publisher,ImageS,ImageM,ImageL) 
        ...
        logger.exception("add books error: {}".format(e)) # <-- 修改：修正日志信息
